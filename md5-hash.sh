...
# bismarck-otto 2026-08-12 to calculate hash with md5-hash.sh
...
: <<'CONFIG_COMMENT'
# /usr/share/applications/md5-hash.desktop

[Desktop Entry]
Version=1.0
Type=Application
Name=Calculate MD5 Hash
Comment=Calculate a MD5 hash and copy it to the clipboard
Exec=/opt/hash/md5-hash.sh
...
# /usr/share/Thunar/sendto/md5-hash.desktop

[Desktop Entry]
Version=1.0
Type=Application
Name=Calculate MD5 Hash
Comment=Calculate MD5 hash and copy it to the clipboard
TryExec=/opt/hash/md5-hash.sh
Exec=/opt/hash/md5-hash.sh %F
Icon=sha256-hash-yellow-padlock
Terminal=false
StartupNotify=true
...
<Menu>
  ...
  <Menu>
    ...
    <Include>
      <Filename>md5-hash.desktop</Filename>
    </Include>
...
#       /opt/hash/md5-hash.sh
#
#    No argument:
#    Open GUI file chooser.
#
# 2. Thunar -> Send To -> Calculate MD5 Hash
#
#       /opt/hash/md5-hash.sh "/path/to/file"
#
...
if (( $# == 0 )); then
    ...
    FILE="$(
        zenity \
            --file-selection \
            --title="Select file to calculate MD5 hash" \
            2>/dev/null
    )" || exit 0
    ...
    show_error "Calculate MD5 Hash accepts one file at a time.

Please select one file in Thunar and use:

Send To → Calculate MD5 Hash"
...
# ------------------------------------------------------------
# Validate calculated MD5
# ------------------------------------------------------------

if [[ ! "$HASH" =~ ^[0-9a-f]{32}$ ]]; then
    show_error "Failed to calculate a valid MD5 hash."
    exit 1
fi
...
#
#     md5:<32-character-lowercase-hash>
#
# No spaces.
# No newline.
# ------------------------------------------------------------

CLIPBOARD_TEXT="md5:${HASH}"

if ! printf '%s' "$CLIPBOARD_TEXT" |
        xclip -selection clipboard
then
    show_error "The MD5 hash was calculated, but it could not be copied to the clipboard."
    exit 1
fi
...
HASH_FIRST="${HASH:0:16}"
HASH_SECOND="${HASH:16:16}"
...
MESSAGE="MD5 hash of ${ESCAPED_FILE_NAME}

...
# bismarck-otto 2026-08-12 to calculate hash with sha512-hash.sh
...
: <<'CONFIG_COMMENT'
# /usr/share/applications/sha512-hash.desktop

[Desktop Entry]
Version=1.0
Type=Application
Name=Calculate SHA512 Hash
Comment=Calculate a SHA512 hash and copy it to the clipboard
Exec=/opt/hash/sha512-hash.sh
...
# /usr/share/Thunar/sendto/sha512-hash.desktop

[Desktop Entry]
Version=1.0
Type=Application
Name=Calculate SHA512 Hash
Comment=Calculate SHA512 hash and copy it to the clipboard
TryExec=/opt/hash/sha512-hash.sh
Exec=/opt/hash/sha512-hash.sh %F
Icon=sha256-hash-yellow-padlock
Terminal=false
StartupNotify=true
...
<Menu>
  ...
  <Menu>
    ...
    <Include>
      <Filename>sha512-hash.desktop</Filename>
    </Include>
...
#       /opt/hash/sha512-hash.sh
#
#    No argument:
#    Open GUI file chooser.
#
# 2. Thunar -> Send To -> Calculate SHA512 Hash
#
#       /opt/hash/sha512-hash.sh "/path/to/file"
#
...
if (( $# == 0 )); then
    ...
    FILE="$(
        zenity \
            --file-selection \
            --title="Select file to calculate SHA512 hash" \
            2>/dev/null
    )" || exit 0
    ...
    show_error "Calculate SHA512 Hash accepts one file at a time.

Please select one file in Thunar and use:

Send To → Calculate SHA512 Hash"
...
# ------------------------------------------------------------
# Validate calculated SHA512
# ------------------------------------------------------------

if [[ ! "$HASH" =~ ^[0-9a-f]{128}$ ]]; then
    show_error "Failed to calculate a valid SHA512 hash."
    exit 1
fi
...
#
#     sha512:<128-character-lowercase-hash>
#
# No spaces.
# No newline.
# ------------------------------------------------------------

CLIPBOARD_TEXT="sha512:${HASH}"

if ! printf '%s' "$CLIPBOARD_TEXT" |
        xclip -selection clipboard
then
    show_error "The SHA512 hash was calculated, but it could not be copied to the clipboard."
    exit 1
fi
...
HASH_THIRD="${HASH:64:32}"
HASH_FOURTH="${HASH:96:32}"
...
LINE3="$(format_pairs "$HASH_THIRD")"
LINE4="$(format_pairs "$HASH_FOURTH")"
...
MESSAGE="SHA512 hash of ${ESCAPED_FILE_NAME}
